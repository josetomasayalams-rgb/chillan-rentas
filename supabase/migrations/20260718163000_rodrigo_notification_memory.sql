-- Memoria independiente de avisos de conserjería para Rodrigo.
-- Aditiva: no modifica la memoria histórica de Beatriz.
...
create policy "public read" on public.rodrigo_notification_batches
  for select using (true);
create policy "public write" on public.rodrigo_notification_batches
  for all using (true) with check (true);
create policy "public read" on public.rodrigo_notifications
  for select using (true);
create policy "public write" on public.rodrigo_notifications
  for all using (true) with check (true);
create policy "public read" on public.rodrigo_notification_events
  for select using (true);
create policy "public write" on public.rodrigo_notification_events
  for all using (true) with check (true);
...
begin
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      ...
      and tablename = 'rodrigo_notifications'
  ) then
  ...
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      and schemaname = 'public'
      and tablename = 'rodrigo_notification_batches'
  ) then
